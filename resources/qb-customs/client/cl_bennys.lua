...
RegisterNetEvent('event:control:bennys1', function(useID)
            ...
            for k, v in pairs(bennyGarages) do
				...
				if GetPedInVehicleSeat(GetVehiclePedIsIn(PlayerPedId()), -1) == PlayerPedId() then
					if (v.useJob and isAuthorized((QBCore.Functions.GetPlayerData().job.name), k)) or not v.useJob then
					    TriggerEvent('event:control:bennys1', k)
					else
...
--         if job == "police" and IsPedInAnyPoliceVehicle(PlayerPedId()) and IsPedInAnyVehicle(plyPed, false) then
--             local plyPos = GetEntityCoords(plyPed)
